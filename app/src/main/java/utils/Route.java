...
public class Route {
	...
	public boolean drawRoute(GoogleMap map, Context c,
			ArrayList<LatLng> points, boolean withIndications, String language,
			boolean optimize) {
		mMap = map;
		context = c;
		lang = language;
		if (points.size() == 2) {
			String url = makeURL(points.get(0).latitude,
					points.get(0).longitude, points.get(1).latitude,
					points.get(1).longitude, "driving");
	...
	public boolean drawRoute(GoogleMap map, Context c,
			ArrayList<LatLng> points, String language, boolean optimize) {
		mMap = map;
		context = c;
		lang = language;
		if (points.size() == 2) {
	...
	public boolean drawRoute(GoogleMap map, Context c,
			ArrayList<LatLng> points, String mode, boolean withIndications,
	...
	public void drawRoute(GoogleMap map, Context c, LatLng source, LatLng dest,
			String mode, boolean withIndications, String language) {
		mMap = map;
		context = c;
	...
	private String makeURL(ArrayList<LatLng> points, String mode,
			boolean optimize) {
	...
	private void drawPath(String result, boolean withSteps) {
		...
		try {
			...
			if (withSteps) {
				...
				BookRideActivity.layoutVisibility(View.VISIBLE, View.GONE);

			}

		} catch (JSONException e) {
			// Toast.makeText(context, "Enter Addess Properly",
			// Toast.LENGTH_SHORT)
			// .show();
		}
	}
